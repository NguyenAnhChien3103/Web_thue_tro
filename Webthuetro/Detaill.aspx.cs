...
namespace Webthuetro
{
    public partial class Detaill : System.Web.UI.Page
    {
        ...
        private void LoadComments(string baiDangId)
        {
            ...
            //               WHERE b.ID_BAIDANG = @baiDangId";
            string sql = @"SELECT b.ID_BAIDANG, d.ID_DANHGIA, d.NOIDUNG, d.NGAYDANHGIA, d.HINHANH, d.USERNAME, u.TENKHACHHANG
FROM BAIDANG b
JOIN DANHGIA d ON b.ID_BAIDANG = d.ID_BAIDANG
JOIN USERR u ON d.USERNAME = u.USERNAME
WHERE b.ID_BAIDANG = @baiDangId;";
            using (SqlConnection connection = create.getConnection())
            {
                connection.ConnectionString = create.connect1; // Ensure the ConnectionString is set
        ...
        //    foreach (DataRow row in dt.Rows)
        //    {
        //        string imageHtml = string.IsNullOrEmpty(row["HINHANH"].ToString()) ? "" : $"<img src='/imgchitiet/{row["HINHANH"]}' alt='Comment Image' />";
        ...
        private string GenerateCommentsHtml(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                DateTime ngayDanhGia = Convert.ToDateTime(row["NGAYDANHGIA"]);
                string timeAgo = CalculateTimeAgo(ngayDanhGia);

                htmlString += $@"
                <div style='margin: 10px 0px ;border: #ccc 2px solid;
                    padding: 5px;
                    box-shadow: #ddd 0 5px 10px 0px;
                    border-radius: 5px;display:flex; justify-content: space-between;'>
                    <div>
                    <h4 style='margin-bottom:10px;font-size:20px'>{row["TENKHACHHANG"]}</h4>
                    <p>{row["NOIDUNG"]}</p>
                    <div class='image-container' style='display:block;'>
                        <img src='/imgchitiet/{row["HINHANH"]}' alt='Uploaded Image' style='width:300px;height:250px;'>
                    </div>
                    </div>
                    <div>Đăng {timeAgo}</div>
                </div>"
        ...
        protected void btnSubmitComment_Click(object sender, EventArgs e)
        {
            ...
            if (FileUpload1.HasFile)
            {
                try
                {
                    ...
                    string folderPath = Server.MapPath("~/imgchitiet");
                ...
                {
                    ShowAlert($"Đã xảy ra lỗi: {ex.Message}");
                    return;
                }
            }
